...
def fold_rfc8792_block(text: str, width: int) -> str:
    """
    Fold a single logical block of text into multiple physical lines.

    - Any line longer than `width` is split into multiple physical lines.
    - No backslashes are added.
    ...
    for line in lines:
        ...
        # Maximum content per wrapped line (no '\' reserved)
        max_part_len = width - indent_len
        if max_part_len <= 0:
            # Pathological case where indentation alone exceeds width
            out_lines.append(line)
            continue

        # Wrap content into chunks
...
def main():
    if len(sys.argv) != 2:
        print("Usage: fold_markdown.py <markdown-filename>")
        sys.exit(1)
